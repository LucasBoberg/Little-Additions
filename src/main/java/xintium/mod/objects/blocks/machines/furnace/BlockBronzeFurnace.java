...
public class BlockBronzeFurnace extends BlockBase implements ITileEntityProvider {

    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    public static final PropertyBool RUNNING = PropertyBool.create("running");

    public BlockBronzeFurnace(String name) {
    ...
    public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
        return new ItemStack(BlockInit.BRONZE_FURNACE);
    }

    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if(!worldIn.isRemote) {
            playerIn.openGui(Main.instance, ConfigHandler.GUI_BRONZE_FURNACE, worldIn, pos.getX(), pos.getY(), pos.getZ());
        }
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        if(active) worldIn.setBlockState(pos, BlockInit.BRONZE_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(RUNNING, true), 3);
        else worldIn.setBlockState(pos, BlockInit.BRONZE_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(RUNNING, false), 3);
    ...
    private TileEntityBronzeFurnace getTile(World worldIn, BlockPos pos) {
        return (TileEntityBronzeFurnace) worldIn.getTileEntity(pos);
    }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        TileEntityBronzeFurnace tileEntityBronzeFurnace = (TileEntityBronzeFurnace) worldIn.getTileEntity(pos);
        InventoryHelper.dropInventoryItems(worldIn, pos, tileEntityBronzeFurnace);
